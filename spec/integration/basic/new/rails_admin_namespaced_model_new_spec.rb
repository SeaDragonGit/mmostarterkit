...
describe 'MmoStarterKit Namespaced Model New' do

  subject { page }

  describe 'AbstractModel#to_param' do
    it 'turns namespaces into prefixes with ~' do
      expect(MmoStarterKit::AbstractModel.new('Cms::BasicPage').to_param).to eq('cms~basic_page')
    end
  end

  describe 'ApplicationController#to_model_name' do
    it 'turns cms~basic_page into Cms::BasicPage' do
      expect(MmoStarterKit::ApplicationController.new.to_model_name('cms~basic_page')).to eq('Cms::BasicPage')
    end
  end
