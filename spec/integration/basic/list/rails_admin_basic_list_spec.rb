...
describe 'MmoStarterKit Basic List' do
  ...
  describe 'GET /admin/player as list' do
    it "shows \"List of Models\", should show filters and should show column headers" do
      MmoStarterKit.config.default_items_per_page = 1
      ...
      should have_no_selector("a[href^='/mmo_starter_kit/main/list']")
    end
  end
  ...
  describe 'GET /admin/player with 3 pages, page 2' do
    before do
      MmoStarterKit.config.default_items_per_page = 1
      items_per_page = MmoStarterKit.config.default_items_per_page
