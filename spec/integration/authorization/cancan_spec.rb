...
class Ability
  ...
  def initialize(user)
    can :access, :mmo_starter_kit if user.roles.include? :admin
...
describe 'MmoStarterKit CanCan Authorization' do
  ...
  before do
    ...
    @player_model = MmoStarterKit::AbstractModel.new(Player)
  ...
  describe 'with create and read player role' do
    ...
    it 'GET /admin/player/new should render and create record upon submission' do
      ...
      @player = MmoStarterKit::AbstractModel.new('Player').first
  ...
  describe 'with a custom admin ability' do
    before do
      MmoStarterKit.config { |c| c.authorize_with :cancan, AdminAbility }
      @user = FactoryGirl.create :user
      login_as @user
    end
