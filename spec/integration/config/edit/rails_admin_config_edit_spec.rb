...
describe 'MmoStarterKit Config DSL Edit Section' do
  ...
  describe " a field with 'format' as a name (Kernel function)" do
    ...
    it 'is updatable without any error' do
      ...
      @record = MmoStarterKit::AbstractModel.new('FieldTest').first
  ...
  describe 'field groupings' do
    ...
    describe 'help' do
      before do
        ...
        MmoStarterKit.config.included_models = [HelpTest, Team]
      end
      ...
      it 'shows help section if present' do
        MmoStarterKit.config HelpTest do
          edit do
            group :default do
  ...
  describe 'bindings' do
    it 'is present at creation time' do
      ...
      MmoStarterKit.config Category do
  ...
  describe 'fields which are nullable and have AR validations', active_record: true do
    ...
    it 'is required' do
      ...
      field = MmoStarterKit.config('Draft').edit.fields.detect { |f| f.name == :notes }
      ...
      field = MmoStarterKit.config('Draft').edit.fields.detect { |f| f.name == :date }
      expect(field.properties.nullable?).to be_true
      ...
      field = MmoStarterKit.config('Draft').edit.fields.detect { |f| f.name == :round }
      expect(field.properties.nullable?).to be_true
      expect(field.required?).to be_true

      # team.revenue is nullable in the schema but has an AR
      # validates_numericality_of validation that allows nil
      field = MmoStarterKit.config('Team').edit.fields.detect { |f| f.name == :revenue }
      expect(field.properties.nullable?).to be_true
      expect(field.required?).to be_false
    end
  end
