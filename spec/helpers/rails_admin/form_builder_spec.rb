...
describe 'MmoStarterKit::FormBuilder' do
  describe '#generate' do
    before do
      allow(helper).to receive(:authorized?).and_return(true)
      (@object = Player.new).save
      @builder = MmoStarterKit::FormBuilder.new(:player, @object, helper, {})
    ...
    it 'does not add additional error div from default ActionView::Base.field_error_proc' do
      expect(@builder.generate(action: :create, model_config: MmoStarterKit.config(Player))).not_to have_css('.field_with_errors')
      expect(@builder.generate(action: :create, model_config: MmoStarterKit.config(Player))).to have_css('.control-group.error')
    end
  end
  ...
  describe '#object_infos' do
    before do
      ...
      @builder = MmoStarterKit::FormBuilder.new(:fan, @object, helper, {})
    end
