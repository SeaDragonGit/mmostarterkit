...
class TestAbility
  ...
  def initialize(user)
    can :access, :mmo_starter_kit
    can :edit, FieldTest
    cannot :edit, FieldTest, string_field: 'dangerous'
  end
end

describe MmoStarterKit::ApplicationHelper do
  describe '#authorized?' do
    before do
      allow(MmoStarterKit.config).to receive(:_current_user).and_return(FactoryGirl.create(:user))
      helper.instance_variable_set('@authorization_adapter', MmoStarterKit::AUTHORIZATION_ADAPTERS[:cancan].new(MmoStarterKit.config, TestAbility))
    end

    it 'doesn\'t test unpersisted objects' do
      am = MmoStarterKit.config(FieldTest).abstract_model
  ...
  describe 'with #authorized? stubbed' do
    ...
    describe '#current_action?' do
      it 'returns true if current_action, false otherwise' do
        @action = MmoStarterKit::Config::Actions.find(:index)

        expect(helper.current_action?(MmoStarterKit::Config::Actions.find(:index))).to be_true
        expect(helper.current_action?(MmoStarterKit::Config::Actions.find(:show))).not_to be_true
      end
    end
    ...
    describe '#action' do
      ...
      it 'returns only visible actions, passing all bindings' do
        ...
        expect(helper.action(:test_bindings, MmoStarterKit::AbstractModel.new(Team), Team.new)).to be
        expect(helper.action(:test_bindings, MmoStarterKit::AbstractModel.new(Team), Player.new)).to be_nil
        expect(helper.action(:test_bindings, MmoStarterKit::AbstractModel.new(Player), Team.new)).to be_nil
      end
    end

    describe '#actions' do
      it 'returns actions by type' do
        abstract_model = MmoStarterKit::AbstractModel.new(Player)
      ...
      it 'only returns visible actions, passing bindings correctly' do
        ...
        expect(helper.actions(:all, MmoStarterKit::AbstractModel.new(Team), Team.new).collect(&:custom_key)).to eq([:test_bindings])
        expect(helper.actions(:all, MmoStarterKit::AbstractModel.new(Team), Player.new).collect(&:custom_key)).to eq([])
        expect(helper.actions(:all, MmoStarterKit::AbstractModel.new(Player), Team.new).collect(&:custom_key)).to eq([])
      end
    end
    ...
    describe '#wording_for' do
      ...
      it 'passes correct bindings' do
        expect(helper.wording_for(:title, :edit, MmoStarterKit::AbstractModel.new(Team), Team.new(name: 'the avengers'))).to eq("Edit Team 'the avengers'")
      end

      it 'defaults correct bindings' do
        @action = MmoStarterKit::Config::Actions.find :edit
        @abstract_model = MmoStarterKit::AbstractModel.new(Team)
      ...
      it 'does not try to use the wrong :label_metod' do
        @abstract_model = MmoStarterKit::AbstractModel.new(Draft)
        @object = Draft.new

        expect(helper.wording_for(:link, :new, MmoStarterKit::AbstractModel.new(Team))).to eq('Add a new Team')
      end
    ...
    describe '#breadcrumb' do
      it 'gives us a breadcrumb' do
        @action = MmoStarterKit::Config::Actions.find(:edit, abstract_model: MmoStarterKit::AbstractModel.new(Team), object: Team.new(name: 'the avengers'))
    ...
    describe '#menu_for' do
      it 'passes model and object as bindings and generates a menu, excluding non-get actions' do
        ...
        @action = MmoStarterKit::Config::Actions.find :show
        @abstract_model = MmoStarterKit::AbstractModel.new(Team)
        @object = Team.new(name: 'the avengers')
      ...
      it 'excludes non-get actions' do
        ...
        @action = MmoStarterKit::Config::Actions.find :dashboard
    ...
    describe '#main_navigation' do
      ...
      it 'shows children of hidden models' do # https://github.com/sferik/mmo_starter_kit/issues/978
        MmoStarterKit.config do |config|
      ...
      it 'orders' do
        ...
        MmoStarterKit.config(Comment) do
          weight 1
        end
    ...
    describe '#bulk_menu' do
      it 'includes all visible bulkable actions' do
        ...
        @action = MmoStarterKit::Config::Actions.find :index
        result = helper.bulk_menu(MmoStarterKit::AbstractModel.new(Team))
        expect(result).to match('zorg_action')
        expect(result).to match('blub')

        expect(helper.bulk_menu(MmoStarterKit::AbstractModel.new(Player))).not_to match('blub')
      end
    end
  end
end
