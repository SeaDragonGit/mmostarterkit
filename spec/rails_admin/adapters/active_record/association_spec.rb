...
describe 'MmoStarterKit::Adapters::ActiveRecord::Association', active_record: true do
  before :all do
    ...
    @blog = MmoStarterKit::AbstractModel.new(ARBlog)
    @post = MmoStarterKit::AbstractModel.new(ARPost)
    @category = MmoStarterKit::AbstractModel.new(ARCategory)
    @user = MmoStarterKit::AbstractModel.new(ARUser)
    @profile = MmoStarterKit::AbstractModel.new(ARProfile)
    @comment = MmoStarterKit::AbstractModel.new(ARComment)
  end
  ...
  describe 'polymorphic belongs_to association' do
    before { allow(MmoStarterKit::Config).to receive(:models_pool).and_return(%w[ARBlog ARPost ARCategory ARUser ARProfile ARComment]) }
