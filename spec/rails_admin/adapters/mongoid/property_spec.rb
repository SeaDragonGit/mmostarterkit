...
describe 'MmoStarterKit::Adapters::Mongoid::Property', mongoid: true do
  subject { MmoStarterKit::AbstractModel.new(FieldTest).properties.detect { |p| p.name == field } }
  ...
  describe '#length_validation_lookup' do
    it 'detects validation length properly' do
      ...
      expect(MmoStarterKit::AbstractModel.new('LengthValiated').properties.last.send(:length_validation_lookup)).to eq(50)
    end
    ...
    it 'does not cause problem with custom validators' do
      ...
      expect(lambda { MmoStarterKit::AbstractModel.new('CustomValiated').properties.last.send(:length_validation_lookup) }).not_to raise_error
    end
  end
end
