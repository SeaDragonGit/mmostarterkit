...
describe 'MmoStarterKit::Adapters::Mongoid::Association', mongoid: true do
  before :all do
    MmoStarterKit::AbstractModel.reset_polymorphic_parents
    ...
    @blog     = MmoStarterKit::AbstractModel.new MongoBlog
    @post     = MmoStarterKit::AbstractModel.new MongoPost
    @category = MmoStarterKit::AbstractModel.new MongoCategory
    @user     = MmoStarterKit::AbstractModel.new MongoUser
    @profile  = MmoStarterKit::AbstractModel.new MongoProfile
    @comment  = MmoStarterKit::AbstractModel.new MongoComment
  end
  ...
  describe 'polymorphic belongs_to association' do
    before { allow(MmoStarterKit::Config).to receive(:models_pool).and_return(%w[MongoBlog MongoPost MongoCategory MongoUser MongoProfile MongoComment]) }
  ...
  describe 'embeds_many association' do
    ...
    it 'raises error when embeds_* is used without accepts_nested_attributes_for' do
      ...
      expect(lambda { MmoStarterKit::AbstractModel.new(MongoEmbedsOne).associations.first.nested_options }).to raise_error(RuntimeError,
                                                                                                                        "Embbeded association without accepts_nested_attributes_for can't be handled by MmoStarterKit,\nbecause embedded model doesn't have top-level access.\nPlease add `accepts_nested_attributes_for :mongo_embedded' line to `MongoEmbedsOne' model.\n"
      )
      expect(lambda { MmoStarterKit::AbstractModel.new(MongoEmbedsMany).associations.first.nested_options }).to raise_error(RuntimeError,
                                                                                                                         "Embbeded association without accepts_nested_attributes_for can't be handled by MmoStarterKit,\nbecause embedded model doesn't have top-level access.\nPlease add `accepts_nested_attributes_for :mongo_embeddeds' line to `MongoEmbedsMany' model.\n"
      )
      expect(lambda { MmoStarterKit::AbstractModel.new(MongoRecursivelyEmbedsOne).associations.first.nested_options }).not_to raise_error
      expect(lambda { MmoStarterKit::AbstractModel.new(MongoRecursivelyEmbedsMany).associations.first.nested_options }).not_to raise_error
    end
    ...
    it 'works with inherited embeds_many model' do
      ...
      expect(lambda { MmoStarterKit::AbstractModel.new(MongoEmbedsChild).associations }).not_to raise_error
    end
  end
end
