...
describe MmoStarterKit::MainController do
  describe '#dashboard' do
    ...
    it 'shows statistics by default' do
      expect(MmoStarterKit.config(Player).abstract_model).to receive(:count).and_return(0)
    ...
    it 'does not show statistics if turned off' do
      ...
      expect(MmoStarterKit.config(Player).abstract_model).not_to receive(:count)
      controller.dashboard
    end
  end
  ...
  describe '#get_sort_hash' do
    context 'options sortable is a hash' do
      ...
      it 'returns the option with no changes' do
        ...
        expect(controller.send(:get_sort_hash, MmoStarterKit.config(Player))).to eq(sort: :"team.name", sort_reverse: true)
      end
    end
    ...
    it 'works with belongs_to associations with label method virtual' do
      ...
      expect(controller.send(:get_sort_hash, MmoStarterKit.config(Category))).to eq(sort: 'categories.parent_category_id', sort_reverse: true)
    end

    context 'using mongoid, not supporting joins', mongoid: true do
      it 'gives back the remote table with label name' do
        controller.params = {sort: 'team', model_name: 'players'}
        expect(controller.send(:get_sort_hash, MmoStarterKit.config(Player))).to eq(sort: 'players.team_id', sort_reverse: true)
      end
    end
    ...
    context 'using active_record, supporting joins', active_record: true do
      it 'gives back the local column'  do
        controller.params = {sort: 'team', model_name: 'players'}
        expect(controller.send(:get_sort_hash, MmoStarterKit.config(Player))).to eq(sort: 'teams.name', sort_reverse: true)
      end
    end
  end
  ...
  describe '#list_entries called from view' do
    ...
    it 'paginates' do
      expect(controller.list_entries(MmoStarterKit.config(Team), :index, nil, false).to_a.length).to eq(21)
      expect(controller.list_entries(MmoStarterKit.config(Team), :index, nil, true).to_a.length).to eq(20)
    end
  end
  ...
  describe '#list_entries called with bulk_ids' do
    ...
    it 'does not paginate' do
      expect(controller.list_entries(MmoStarterKit.config(Team), :bulk_delete).to_a.length).to eq(21)
    end
  end
  ...
  describe 'sanitize_params_for!' do
    context 'in France' do
      ...
      it 'sanitize params recursively in nested forms' do
        ...
        MmoStarterKit.config NestedFieldTest do
          configure :created_at do
            show
          end
        end
        ...
        controller.send(:sanitize_params_for!, :create, MmoStarterKit.config(FieldTest), controller.params['field_test'])
    ...
    it 'allows for polymorphic associations parameters' do
      ...
      controller.send(:sanitize_params_for!, :create, MmoStarterKit.config(Comment), controller.params['comment'])
      expect(controller.params).to eq(
        'comment' => {
          'commentable_id' => 'test',
          'commentable_type' => 'test'
