  ...
  module Extensions
    module History
      class AuditingAdapter
        ...
        def latest
          ::MmoStarterKit::History.latest
        end

        def delete_object(object, model, user)
          ::MmoStarterKit::History.create_history_item('delete', object, model, user)
        end

        def update_object(object, model, user, changes)
          ::MmoStarterKit::History.create_history_item(changes.collect { |k, v| "#{k}: #{v.collect(&:inspect).join(' -> ')}" }, object, model, user)
        end

        def create_object(object, model, user)
          ::MmoStarterKit::History.create_history_item('new', object, model, user)
        end

        def listing_for_model(model, query, sort, sort_reverse, all, page, per_page = (MmoStarterKit::Config.default_items_per_page || 20))
          ::MmoStarterKit::History.history_for_model(model, query, sort, sort_reverse, all, page, per_page)
        end
        ...
        def listing_for_object(model, object, query, sort, sort_reverse, all, page, per_page = (MmoStarterKit::Config.default_items_per_page || 20))
          ::MmoStarterKit::History.history_for_object(model, object, query, sort, sort_reverse, all, page, per_page)
        end
      end
    end
  end
