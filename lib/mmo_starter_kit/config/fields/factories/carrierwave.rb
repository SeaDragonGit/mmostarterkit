...
MmoStarterKit::Config::Fields.register_factory do |parent, properties, fields|
  ...
  if defined?(::CarrierWave) && (model).kind_of?(CarrierWave::Mount) && model.uploaders.include?(attachment_name = properties.name.to_s.chomp('_file_name').to_sym)
    ...
    field = MmoStarterKit::Config::Fields::Types.load(:carrierwave).new(parent, attachment_name, properties)
    fields << field
    children_fields = []
    columns.each do |children_column_name|
      if child_properties = parent.abstract_model.properties.detect { |p| p.name.to_s == children_column_name.to_s }
        children_field = fields.detect { |f| f.name == children_column_name } || MmoStarterKit::Config::Fields.default_factory.call(parent, child_properties, fields)
