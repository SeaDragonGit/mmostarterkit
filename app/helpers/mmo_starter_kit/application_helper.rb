  ...
  module ApplicationHelper
    ...
    def navigation(nodes_stack, nodes, level = 0)
      nodes.collect do |node|
        ...
        url         = url_for(action: :index, controller: 'mmo_starter_kit/main', model_name: model_param)
        ...
        parent_actions.collect do |a|
          ...
          content_tag(:li, class: current_action?(a, am, o) && 'active') do
            crumb = if a.http_methods.include?(:get)
              link_to url_for(action: a.action_name, controller: 'mmo_starter_kit/main', model_name: am.try(:to_param), id: (o.try(:persisted?) && o.try(:id) || nil)), class: 'pjax' do
      ...
      actions.collect do |action|
        ...
        %(
          <li title="#{wording if only_icon}" rel="#{'tooltip' if only_icon}" class="icon #{action.key}_#{parent}_link #{'active' if current_action?(action)}">
            <a class="#{action.pjax? ? 'pjax' : ''}" href="#{url_for(action: action.action_name, controller: 'mmo_starter_kit/main', model_name: abstract_model.try(:to_param), id: (object.try(:persisted?) && object.try(:id) || nil))}">
